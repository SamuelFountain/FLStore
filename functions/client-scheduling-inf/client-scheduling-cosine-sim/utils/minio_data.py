...
# clients_info_json_file = 'resnet18_cfarclients-10-of-250_rounds-2000_epochs-3_batchSize-20.json'
...
def create_model_by_copy(model_name, layers_data):
    ...
    if model_name in [
        ...
        if model_func is None:
            ...
            return None
        model = model_func()
    elif model_name.startswith("swin"):  # Check if the model is a Swin V2 model
...
def readAndCreateModels(round):
    ...
    try:
        ...
        for c in clients:
            ...
            client_models[c] = create_model_by_copy("efficientnet_v2_s", client_model)
            end_model_create_time = time.time()
            model_creation_times.append(end_model_create_time - start_model_create_time)
