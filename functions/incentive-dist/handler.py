...
def main():
    ...
    for round_no in range(1, 6):
        ...
        if PER_CLIENT:
            for client_id, model in client2models.items():
                # Model inference
                # accuracies.append(model_inference(dataset, model))
                accuracies[client_id] = model_inference(dataset, model)
            average_accuracy = sum(accuracies.values()) / len(accuracies)
            for client_id, model in client2models.items():
                if not client_id in tokens_per_client:
                    tokens_per_client[client_id] = tokens_per_round * (
                        accuracies[client_id] - average_accuracy
                    )
                else:
                    tokens_per_client[client_id] += tokens_per_round * (
                        accuracies[model] - average_accuracy
                    )
        else:
            # Model inference
            # accuracies.append(model_inference(dataset, models[0]))
            accuracies[client_id] = model_inference(dataset, models[0])
            tokens_per_client[client_id] = tokens_per_round
