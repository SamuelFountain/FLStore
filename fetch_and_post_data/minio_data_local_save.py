...
clients_info_json_file = os.path.join(
    ...
    "utils/json_for_Ahmad/resnet18_cfarclients-10-of-250_rounds-2000_epochs-3_batchSize-20.json",
)
...
def readAndCreateModels(round):
    ...
    Returns a dictionary in the format:
      { round_key: { client_id: { layer_name: object_data, ... }, ... } }
    """
    ...
    try:
        ...
        if round_key not in mapping:
            print(
                f"Round {round_key} not found in JSON mapping. Using last available round."
            )
        ...
        for client_id, layer_map in mapping[round_key].items():
            ...
            for layer_name, layer_loc in layer_map.items():
                ...
                # print(f"Fetching layer '{layer_name_str}' for client '{client_id_str}' using key '{layer_loc}'")
                client_model[layer_name_str] = read_key_value(
                    client, bucket_name, layer_loc
                )
            client_models[client_id_str] = client_model
...
def save_data_locally_for_rounds(rounds, zip_filename="local_data.zip"):
    """
...
def load_local_data(zip_filename="local_data.zip"):
